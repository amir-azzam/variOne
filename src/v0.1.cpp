...
// === BUTTONS ===
// Layout: LEFT=Back(q)  UP=Up(w)  RIGHT=Select(e)  DOWN=Down(s)
#define BTN_LEFT  14   // Back  → q
#define BTN_UP    26   // Up    → w
#define BTN_RIGHT 32   // Select→ e
#define BTN_DOWN  33   // Down  → s
...
void drawAbout() {
  ...
  drawHeader("About");
  display.setFont(u8g2_font_6x10_tr);
  display.drawStr(4, 30, "VariOne v0.1");
    ...
    case STATE_MENU:
      if (input == 'w' && menuIndex > 0) menuIndex--;
        ...
        switch (menuIndex) {
          ...
          case 5: currentState = STATE_SUBGHZ; break;
          case 6: currentState = STATE_NFC; break;
          case 7: currentState = STATE_IR; break;
          case 8: currentState = STATE_ABOUT; break;
        }
      }
      break;
...
void setup() {
  Serial.begin(115200);
  Serial.println("VariOne v0.1 booting...");
  ...
  display.setFont(u8g2_font_6x10_tr);
  display.drawStr(15, 48, "Initializing...");
  display.sendBuffer();
  delay(1500);

  Serial.println("Ready. Buttons: UP=GPIO26 DN=GPIO34 OK=GPIO32 BK=GPIO14");
  Serial.println("Serial still works: w=up s=down e=select q=back");
  currentState = STATE_MENU;
}
...
void loop() {
  ...
  // Input: physical buttons + serial (both work simultaneously)
  char input = 0;
